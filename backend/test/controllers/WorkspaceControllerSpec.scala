...
class WorkspaceControllerSpec
  ...
  "WorkspaceController" should {
    ...
    "return 400 when creating workspace with existing name" in {
      val createJson = Json.obj(
        "name" -> "Test Workspace",
        "description" -> "Integration test"
      )
      ...
      (contentAsJson(result) \ "message")
        .as[String] mustBe "Duplicate workspace name"
    }
    ...
    "fail when updating unexisting workspace" in {
      val updateJson = Json.obj(
        "name" -> "OK Workspace",
        "description" -> "Updated description"
      )
      ...
      // Create request with JSON body and cookie
      // Use the workspaceId from the created workspace
      val request = FakeRequest(PUT, s"/api/workspaces/0")
        .withJsonBody(updateJson)
        .withCookies(Cookie(cookieName, token))
    ...
    "return 400 when updating workspace with existing name" in {
      val updateJson = Json.obj(
        "name" -> "Updated Workspace",
        "description" -> "Updated description"
      )
      ...
      ex.message must include("Duplicate workspace name")
    }
